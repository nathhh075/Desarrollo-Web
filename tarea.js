...
// IVA 19% (función flecha)
const calcularIVA = (monto, tasa = 0.19) => Math.round(monto * tasa);
...
function procesarPedido(){
    ...
    // por si el user no elige nada
    if (platos.length === 0) {
        console.log("No agregaste ningún plato. Pedido cancelado.\n");
        return;
    }

    // separar por categoría (para ordenar bonito el resumen)
    ...
    // IVA y total final
    let iva         = calcularIVA(total);      // 19% redondeado
    let totalConIVA = total + iva;

    // guardar pedido (agrego iva y totalConIVA, no rompe nada)
    let pedido = { platosModificados, cliente, total, iva, totalConIVA };
    ...
    // promo sigue evaluando sobre el subtotal (como lo tenías)
...
function calcularTotalCuenta() {
  ...
  pedidosCrudos.forEach((p, i) => {
    // por si hay pedidos viejos sin estos campos
    const ivaCalc = (typeof p.iva === 'number') ? p.iva : calcularIVA(p.total);
    const totalConIVA = (typeof p.totalConIVA === 'number') ? p.totalConIVA : p.total + ivaCalc;
    ...
    console.log(`   IVA (19%): $${ivaCalc}`);
    console.log(`   Total con IVA: $${totalConIVA}`);

    // clasificación por monto (con subtotal, como lo tenías)
    if (p.total < 30000) {
      categoriaMonto = "Bajo";
    } else if (p.total <= 80000) {
  ...
  const ingresoDiarioSinIVA = pedidosCrudos.reduce((acc, p) => acc + p.total, 0);
  const ingresoDiarioConIVA = pedidosCrudos.reduce((acc, p) => {
    const ivaCalc = (typeof p.iva === 'number') ? p.iva : calcularIVA(p.total);
    return acc + p.total + ivaCalc;
  }, 0);
